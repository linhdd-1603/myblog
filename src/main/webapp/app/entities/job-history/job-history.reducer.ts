...
export const ACTION_TYPES = {
  SEARCH_JOBHISTORIES: 'jobHistory/SEARCH_JOBHISTORIES',
...
export default (state: JobHistoryState = initialState, action): JobHistoryState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_JOBHISTORIES):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_JOBHISTORIES):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_JOBHISTORIES):
...
const apiSearchUrl = 'api/_search/job-histories';

// Actions

export const getSearchEntities: ICrudSearchAction<IJobHistory> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_JOBHISTORIES,
  payload: axios.get<IJobHistory>(`${apiSearchUrl}?query=${query}`)
});

export const getEntities: ICrudGetAllAction<IJobHistory> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_JOBHISTORY_LIST,
  payload: axios.get<IJobHistory>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});
